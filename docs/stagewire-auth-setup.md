...
3. Keep the default Supabase email templates and SMTP service. StageWire accepts the secure email link, so no paid email provider is required.
4. In Authentication > URL Configuration, set the Site URL to the exact StageWire app origin and add the same origin as an allowed redirect URL.
5. Keep the default one-minute resend window and one-hour link expiry.
6. Copy the project URL and publishable key from the project API settings.
...
- Supabase sends its default secure, one-time email link.
- The link returns a short-lived provider access token to the StageWire origin.
- StageWire immediately removes the token from the browser address and verifies it server-side.
